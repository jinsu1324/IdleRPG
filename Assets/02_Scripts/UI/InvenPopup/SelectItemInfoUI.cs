...
public class SelectItemInfoUI : MonoBehaviour
{
    ...
    [Title("스탯UI 리스트", bold: false)]
    [SerializeField] private List<SelectItemInfoUI_Stat> _statUIList;   // 스탯표시용 UI 리스트
    ...
    private void UpdateStats()
    {
        ...
        // 스탯 Dictionary 순회
        foreach (var kvp in CurrentItem.GetStatDict())
        {
            StatType statType = kvp.Key;
            int value = kvp.Value;

            _statUIList[index].Show(statType, value);
            index++;
        }

        // 나머지 리스트 요소 비활성화
        for (int i = index; i < _statUIList.Count; i++)
        {
            _statUIList[index].Hide();
        }
    }
